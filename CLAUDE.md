...
- **Automatické testy (V0.3.4+):** headless sada `ruby tests/run_all.rb` beží v GitHub Actions na každý push/PR (1776 testov k v0.7.62; lokálne `scripts\run_tests.ps1`, vyžaduje standalone Ruby v `C:\Ruby32-x64`) + **JS sady** (62 sád k v0.7.62) — spúšťať KAŽDÚ zvlášť, bash: `for f in tests/js/test_*.js; do node "$f" || exit 1; done` (POZOR: `node tests/js/test_*.js` spustí len PRVÝ súbor, zvyšok sú preň argumenty; CI ich spúšťa všetky — lokálne tiež všetky, nie len test_expr). In-SketchUp runner `scripts\run_su_tests.ps1` (deploy → inštancia nad kópiou ENGINEtests.skp → poll → výsledok; ~140 scenárov; výsledkový grep až PO dobehu — output sa dopisuje) — spúšťať pri zmenách builderov/observerov; overuje geometriu plán↔model a undo scenáre.
