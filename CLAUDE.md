...
- **materials_migration** — **2A-2: dormantná jednorazová migrácia katalógu na SCHEMA 2** (štandard §7.1): `migrate_to_schema2!` — surové čítanie, kanonická mapa (heuristika len fallback s reportom), deterministický `group_id` (GRP-SHA256, navždy zmrazený), jediná nerozhodnuteľná položka = atomický NO-OP; ostrý beh = nemenná záloha → CAS kontrola bajtov → zápis so schema 2. Žiadna produkčná cesta ju nevolá (aktivácia až 2A-4); `Materials.test_dir_override` = izolácia katalógu VÝHRADNE pre testy.
