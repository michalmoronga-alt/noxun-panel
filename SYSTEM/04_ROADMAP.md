...
- **2A-2** migrácia: explicitná mapa (**SCHVÁLENÁ Michalom 30.7.** — zápisník „2A migračná mapa": zmazať osirotenú pásku „Pracovna doska" ✓, zlúčiť „Halifax Tabakový PD" do skupiny ✓) + heuristika len fallback s reportom, nerozhodnuteľná položka = atomický NO-OP celej migrácie, fixtures testy, kompatibilita starých modelov = zachovanie VŠETKÝCH nezmazaných ID + lookup test (starý .skp → BOM/validácia/rebuild/remap)
