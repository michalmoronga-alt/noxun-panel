...
- **Kedy sa plní:** keď má dielec ABS pásku, ktorej **dekor sa líši od dekoru dosky**. Dekor pásky aj dosky musí byť ZNÁMY (obe v katalógu, dekor neprázdny) — inak sa poznámka
  **nevymýšľa** (neznámy materiál/pásku hlási KONTROLA a oddiel „Riadky vyradené z CSV" v LOGu). Porovnanie je normalizované rovnako, ako väzba materiál↔ABS (`decor_norm_key`:
  medzery preč, case-insensitive). **UNI dosky sa neporovnávajú** — ich „dekor" je pracovný názov (`Korpus UNI`), materiál je neurčený a KONTROLA to už hlási samostatne.
