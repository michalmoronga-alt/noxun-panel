...
- **KOV-C · TASK PACKAGE „NEMENNÉ RECEPTY ZÁSUVIEK A ODVODENÉ DIELCE" (slice C; **v2 z 5.9.2026** — simplification review Michal + Claude nahradil package v13 z PR #300
  ...
  **C2a · príprava aktivácie (bez zmeny výstupov) — ✅ HOTOVÉ (PR #303, v0.9.30, 5.9.2026):** z bodov (b) a (d) nižšie je v maine všetko, čo sa dá spraviť BEZ zapnutia:
  ...
  a oprava = nový `_v2`.
  **C2b · aktivácia — ✅ HOTOVÉ (PR #304, v0.9.31, 5.9.2026):** v maine sú body **(a), (c), (d) zvyšok, (e), (f), (g)** — resolver v `Construction.build_plan` (`drawer_pass`),
  ...
  4. kanál a `drawer_material_id` cestuje vkladacou kartou aj „Nahradiť UNI…" (jeden receptový predikát pre selektor v Štúdiu aj pre náhradu UNI).
  **Rozdelenie po kole 4 (Michal, pravidlo 3 kôl):** UI a hromadné cesty 4. materiálového kanála (výber „Zásuvky" v Štúdiu + preflight per systém, „Nahradiť UNI…",
  vkladacia karta, remap ABS drawer vetva) sa presunuli do SAMOSTATNÉHO PR `feat/kov-c2b-material-kanal`, ktorý sa mergne hneď po #304. V #304 ostáva aktivácia v engine
  + serverové cesty kľúča nutné pre integritu dát (normalize/config, šablóna, delete guard, remap kontrakt). **Vedomé odchýlky:** (1) `Fronts.norm_drawer` NEOVERUJE, či je ref registrovaný —
