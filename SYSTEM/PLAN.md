...
**C · Sekcia Kovanie (zvyšky mini dávky ŠT-3a-3, review #216/#218):**
- **Dvojité kreslenie setov pri každom pushi** (`HWSETS.init` → `hwsRenderAll` a `hwRenderBody` → `hwsRenderAll`) — rozdeliť kontrakt „nastav dáta" vs. „kresli"; k tomu kurzor v editore setu (`hwsRenderSets` bez `activeElement` guardu, vzor `mdhRender`).
- **`rescue` + `abort_operation` zjednotiť** v `handle_map_project`/`reset_project` (výnimka medzi `start_operation` a `commit_operation` nechá operáciu otvorenú; `merge_seed` rescue zas abortuje aj po commite) — krehké.
- **`reset_project` pri `ok == false`** → `resync_sets` namiesto plného pushu (nič sa nezapísalo).
- **`MDH.created` nečistí `HW_Q`/`HW_CAT`** — starý filter sa po pushi vráti.
- **Prázdne zhody Demos hľadania po návrate do sekcie** (kozmetika, priznané v #216/#218).
- **`#hn_unit` bez `keep`** — rozpísaná MJ sa pri plnom pushi prepne na prvú (rovnaký vzor opravy ako P1 #218 pre `#hn_category`, aj s hodnotovým guardom).
...
- **`do_select`/`fresh_collect` (vrátane dedupu) beží aj pre `rule_ref`**, ktorý `collected` nepoužíva — pre-existujúce.
- **Odmietnutý SAVE pravidiel stále repushuje** cez `refresh_studio(bump: false)` = `fresh_collect` s dedupom — tá istá mutácia, ktorú P1 zakázal pri resete; preklopiť na echo sekcie.
...
**E · Post-hoc Codex sweep #186–#227.** Codex bol 21.–24.8. nedostupný a PR #186–#226 prešli slepým reviewer subagentom. Keď má Codex kapacitu, prejsť tie PR spätne — nie kvôli nedôvere v subagenta (chytil o. i. spiacu mínu duplicitných kódov), ale preto, že je to iný pohľad na dávky, ktoré medzitým tvoria základ celej fázy.
