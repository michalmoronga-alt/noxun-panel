...
- **KOV-E · „VÝKLOPY HK top / HL top" (po F; v4 9.9.2026 — po Astra audite [zdroje/next_sessions/KOVANIE_KOVE_AUDIT_2026-09-09_ASTRA.md](zdroje/next_sessions/KOVANIE_KOVE_AUDIT_2026-09-09_ASTRA.md) [4 BLOCKER + 7 FIX + 1 NOTE] a Codex GH #331 [2 P1 + 6 P2]; TRI PR: E1a dáta, E1b pravidlo + brány, E2 UI):**
  ...
  **Definície (Astra FIX 5, otvorené pre Michala):** **KH = výška korpusu BEZ sokla** (`height − floor_height`, nezaokrúhlený Float — Blum KH je korpus, nie výrobná dĺžka
  ...
  **`code_by_param: { "param": "lift_class", "codes": { "22K2300": "347810", … } }`** (serializovaný selektor, string kľúče, `code XOR code_by_nl XOR param_bands XOR
  code_by_param`; chýbajúci kľúč = NEVYRIEŠENÝ člen) a **`quantity_from: "rod_count"`** (celé číslo ≥ 0 z `params`; `rod_extension` explicitne 0/1; chýbajúca/necelá
  hodnota = nevyriešený člen) — `MEMBER_KEYS` + obsahová detekcia `snapshot_std` (lazy bump); **downgrade je FAIL-CLOSED a taký sa aj testuje:** starší čítač
  ...
  **klasifikácia:** nové pole `lift_system` (`hk_top|hl_top`) pri `use_type lift` (whitelist + round-trip guard), `parse_class_head` tretí segment pre `lift` = `lift_system`
  ...
  `/flap` v JEDNEJ dávke (Astra FIX 6); **voľba tmavého setu = `config.hardware_sets` s kľúčom `class:lift|<mode>|<system>@front:<id>/flap` → set_id** (NIE `hardware_overrides` — Codex P2),
  ...
  **E1b PRAVIDLO + BRÁNY (Audit ÁNO — delta Sol pred implementáciou: config čela, nový kind, brány):** config čela `lift.system` (`hk_top` predvolene | `hl_top`,
  CONFIG_SCHEMA 9 → 10 — 9 minula KOV-F1; whitelisty šablón aditívne) + **`lift` do `FRONT_EXTRA_KEYS`** (form.js — bez toho Inspector prepíše HL na HK; Astra FIX 10) ·
  **sklop (`fall`) = závesy ako dvierka:** DRUHÉ seed pravidlo `zavesy-sklop` (rovnaké `bands` + door guardy ako F) s `applies_to: {role: flap, flap_dir: down}` —
  ...
  Michal potvrdí): HK `kh_min 205 · kh_max 600 · kb_max 1800`, HL `kh_min 300 · kh_max 580 · kb_max 1800 · depth_min 264` (vnútorná hĺbka) · **HK: `LF = KH × (weight_kg +
  ...
  **`lift_set_incomplete`** (Astra BLOCKER 1): KAŽDÝ nevyriešený člen setu položky `lift` (chýbajúci kód triedy, `quantity_from` bez hodnoty, chýbajúci set/mapovanie)
  ...
  `lift_dimension_unsupported` (eligibility) · RED `lift_multirow_unsupported` · RED `lift_combo_unsupported` (`hl_top` + Tip-On) · RED **`lift_stale`** (Astra BLOCKER 2):
  `lift_stale_issue` v `Bom.collect` (tretí vzor po `drawer_stale_issue`/`hinge_stale_issue`) — skrinka s čelom `flap_dir up` bez položky `lift` a bez lift konfliktu
  = existujúca zákazka po upgrade (`ensure_project_rules!` seed nedopĺňa) → náprava „Doplniť nové predvoľby + prestavba" · ORANGE `lift_light_front` (pod min: HK
  ...
  · `door_wider_than_high` sa na výklop NEuplatňuje · **plný automat = položky `lift` chránené ako receptové** (Astra FIX 11): `apply_overrides` `disabled`/`quantity`
  ignoruje + ORANGE `lift_override_ignored`, neplatné existujúce overridy normalizácia vyčistí so záznamom v logu · `HardwareRules::STD` bump (čítače od F `std`
  ...
  chýbajúci kód člena → `lift_set_incomplete` blokuje export (nie „krytky samé") · stale skrinka bez položky → `lift_stale` · override `disabled` na `lift` → ignorovaný +
  ORANGE · tmavý set v `config.hardware_sets` prežije prestavbu, reopen, Undo · in-SU `run_kove`. Smoke: výklop 600 × 400 v skrinke 400 → LF ≈ 400 × 3,4 = 1380 → 22K2300 +
  ...
  široká → 2 tyče + predĺženie; KH 250 → RED; sklop → závesy; stará zákazka s výklopom → RED `lift_stale`, „Doplniť nové predvoľby" + prestavba ju zhasne.
