...
- **KOV-E · „VÝKLOPY HK top / HL top" ✅ KOMPLET (po F; v4 9.9.2026 — po Astra audite [zdroje/next_sessions/KOVANIE_KOVE_AUDIT_2026-09-09_ASTRA.md](zdroje/next_sessions/KOVANIE_KOVE_AUDIT_2026-09-09_ASTRA.md) [4 BLOCKER + 7 FIX + 1 NOTE] a Codex GH #331 kolo 1 [2 P1 + 6 P2] + kolo 2 [4 P1 + 1 P2]; TRI PR: E1a dáta, E1b pravidlo + brány, E2 UI):**
  ...
  **`Bom.pre_lift_build?`** (tá istá autorita ako RED `flap_stale`) · **výber setu vrátane tmavého** beží BEZ zmeny klienta (položka `lift` má triedny kľúč, takže
  ...
  spôsobilosť per systém; `max_exclusive` prechádza zberom bezstratovo cez `data-mx`) so **spoločnou Ruby/JS validáciou** — tri nové kritériá (záporný prah tyče, záporná
  hodnota v tabuľke, spôsobilosť s obrátenou výškou) v `rules_validation_parity.json` · **editor setov sa naučil `code_by_param` a `quantity_from`** (stratégia kódu = select
  ...
  serverovej brány `new_shape_members?`** — ochranou ostáva validácia obsahu. **Odchýlky:** štítok „automat" je posledný segment textu riadku, nie samostatný badge (nový
  markup by pribudol bez zisku) · **LF/hmotnosť sa v technickom detaile NEDOPOČÍTAVAJÚ** — na položke uložené nie sú a druhý výpočet tej istej veličiny by sa s automatom
  ...
  in-SU beh sa nekonal: dávka nepridala žiadnu novú zápisovú akciu panela (výber setu ide existujúcou `set_hardware_set`). Uzáver: headless **3643 testov**, **102 JS sád**.
