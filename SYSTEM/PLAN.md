...
- **KOV-C · TASK PACKAGE „RECEPTY, KONTEXT A ODVODENÉ DIELCE ZÁSUVIEK" (slice C; štart po KOV-A/B; **Audit: kolo 1 Astra (6 BLOCKER + 3 FIX-C1 + 3 FIX-C2 + 2 NOTE) + kolo 2 Sol (8/14
  vyriešené, 5 čiastočné, 1 nevyriešené; 3 nové BLOCKER + 3 FIX) + kolo 3 Sol (6/11 vyriešené, 5 čiastočné; 6 nových BLOCKER kontraktovej presnosti + 2 FIX) + kolo 4 Sol (7/8 vyriešené; 1
  BLOCKER sync tyč vs R-06a → tvrdá brána do KOV-D, 1 FIX `locked` spotrebitelia, 3 NOTE) = **AUDIT HOTOVÝ 5.9.2026 (4 kolá, Astra + Sol)**; záznam
  [zdroje/next_sessions/KOVANIE_KOVC_AUDIT_2026-09-05_18.md](zdroje/next_sessions/KOVANIE_KOVC_AUDIT_2026-09-05_18.md); in-SU povinné; **rez C1 → C2a → C2b**):**
  ...
  **Klasifikácia → kľúč receptu (audit 1 B3, záväzná mapa):** `drawer.construction metal → system atira` · `wood → quadro_v6_eb23` · `other → BEZ receptu` (legacy cesta, CONTENT-identická, žiadne
  ...
  (schéma, `recipe_version`, všetky tabuľky, piny, extras, `thickness_supported`) — zmena ktorejkoľvek hodnoty mení digest (audit 3 B5).
  **Projektový snapshot receptov (audit 1 B4 — fail-closed, NIE zrkadlo `ensure_project_rules!`):** kľúč `drawer_recipes` v NOXUN dict modelu `{std, recipe_version, systems}`; snapshot má
  ...
  z disku); `merge_recipes_seed!` = explicitná akcia s diffom (UI v KOV-D) — **aplikácia merge = JEDNA operácia: nový snapshot + prestavba VŠETKÝCH dotknutých čiel cez resolver + prepis
  ...
  **C2a · príprava (audit 1 N14 — produkčný hook NEAKTÍVNY, VŠETKY výstupy CONTENT-identické):** 4. materiálový kanál `:drawer` + UNI migrácia (`ensure_drawer_uni!` samostatná idempotentná —
  ...
  horná dlhá hrana per nový dielec), **persistencia `drawer.recipe` (serverom odvodené, tolerantné — v C2a BEZ bumpu schémy, lebo hook je neaktívny a starší plugin pole len zahodí bez
  výrobného dopadu; Codex #300 P1)**, prenos polí cez `normalize`/zápis/kópiu/šablóny; **`CabinetBuilder::CONFIG_SCHEMA` 4 → 5 sa bumpne AŽ v C2b pri aktivácii** (recipe kovanie `source:
  ...
  len odtlačok, nie celý pack). Testy C2a: CONTENT-identita všetkých výstupov, kolízia šablón, `drawer.recipe` prežije normalizáciu; downgrade test schémy 5 patrí do C2b.
  **C2b · aktivácia (mení výstupy — NARAZ dielce + výsuv + migrácia + brány; VŠETKY zmeny len pre čelá klasifikované ako zásuvka so systémom; ostatné zákazky CONTENT-identické):**
  ...
  D-46 pending-confirmation reuse — pre kanál `:drawer` NIE mechanicky (dnešná všeobecná vetva kontroluje hrúbku čela 18): preflight per systém (Atira len 16, Quadro 16/18; audit 4 NOTE);
  `thickness_ok_for?` pre nové roly; `materialized_part` sa pre drawer roly NEPOUŽÍVA (audit 3 F8: hrúbka vyriešená pred receptom, drawer deskriptor vzniká rovno s finálnou geometriou);
  **hrúbka = vstup receptu** — materiál mimo `thickness_supported` = conflict;
  ...
  (c) **jedna položka kovania** v ÚPLNOM tvare `BuildPlan` (audit 2 F4): `generic_type: slide`, `rule_id: 'recipe:<system>'`, **`owner_part_key: front:<id>/panel`** (identita jednej
  položky na zásuvku aj identita overridu), `quantity: 1`, `rule_quantity: 1`, `production_class` a `manufactured` podľa existujúcich slide položiek, `source: recipe` — **BuildPlan
  `source` enum sa rozšíri o `recipe`** (`plan_schema` bump; dnes len `rule|manual`) a **pôvod položky sa oddelí od príznaku zámku** (audit 3 B3): zámok = `locked: true` na položke (nové
  pole), NIE prepnutie `source` na `manual` ako pri D-93; `rule_nominal_length` je pri `source recipe` povolené; migrácia D-93 (`source manual` + `rule_nominal_length` pod
  `vysuvy-nl-podla-hlbky`) = recipe položka s `locked: true` a zámok v `hardware_overrides`; **spotrebitelia zámku (audit 4 FIX):** `locked` je autorita pre recipe položky —
  `HardwareSets.note_manual` a payloady Nákupu/Inspectora (`payloads.rb`) berú `locked: true` rovnako ako dnešné `source == manual` (značka ručného zámku + pôvodná automatická hodnota
  ostávajú viditeľné), legacy `source manual` pre staré pravidlá sa NEMENÍ; test: migrovaný D-93 zámok je viditeľný v Nákupe aj Inspectore, `params {system, height_variant (Atira: ČÍSELNÉ
  70|144|176) | box_height (Quadro: číselné mm), nominal_length, load, opening, runner_variant}` — jeden kanonický strojový tvar výšky v celom reťazci (Codex #300 P1): výstup resolvera,
