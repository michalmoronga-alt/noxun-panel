...
- **KOV-C · TASK PACKAGE „RECEPTY, KONTEXT A ODVODENÉ DIELCE ZÁSUVIEK" (slice C; štart po KOV-A/B; **Audit: kolo 1 Astra (6 BLOCKER + 3 FIX-C1 + 3 FIX-C2 + 2 NOTE) + kolo 2 Sol (8/14 vyriešené, 5 čiastočné, 1 nevyriešené; 3 nové BLOCKER + 3 FIX) + kolo 3 Sol (6/11 vyriešené, 5 čiastočné; 6 nových BLOCKER kontraktovej presnosti + 2 FIX) ZAPRACOVANÉ 5.9.2026 — kolo 4 (Sol, záverečné) ČAKÁ**; záznam [zdroje/next_sessions/KOVANIE_KOVC_AUDIT_2026-09-05_18.md](zdroje/next_sessions/KOVANIE_KOVC_AUDIT_2026-09-05_18.md); in-SU povinné; **rez C1 → C2a → C2b**):**
  ...
  **C2b · aktivácia (mení výstupy — NARAZ dielce + výsuv + migrácia + brány; VŠETKY zmeny len pre čelá klasifikované ako zásuvka so systémom; ostatné zákazky CONTENT-identické):**
  ...
  D-46 pending-confirmation reuse; `thickness_ok_for?` pre nové roly; `materialized_part` sa pre drawer roly NEPOUŽÍVA (audit 3 F8: hrúbka vyriešená pred receptom, drawer deskriptor vzniká rovno s finálnou geometriou); **hrúbka = vstup receptu** — materiál mimo `thickness_supported` = conflict;
