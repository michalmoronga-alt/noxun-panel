...
- ~~**K1 · D-108 · Smer dekoru per dielec/čelo ako VSTUP**~~ — **HOTOVÉ** (PR #185, **v0.7.23**, 21.8.). Karta dielca má segment **Podľa materiálu / Pozdĺžna / Priečna**; dedený stav ukazuje výsledok a každá voľba nesie v tooltipe výrobný rozmer (2000×250 vs 250×2000). Kontrakt: override žije v `part_overrides['grain_direction']` (enum `length`/`width`), efektívny smer počíta `CabinetBuilder.effective_grain` a **materializuje sa raz do snapshotu dielca**; **rotácia sa nepridala nikde** — vymieňa dĺžku/šírku a dvojice hrán naďalej len VEPO a `fits_on_sheet`. Materiál bez smeru override **ignoruje, ale nemaže**; staré modely bez poľa sa otvorením nezmenia. Overené plným in-SketchUp behom (sekcia `run_k1`).
