...
- **Smer dekoru dielca:** K1 `grain_direction` override / vlastnosť materiálu. Dielec so smerom sa **neotáča**. **Dielec bez smeru** (materiál bez smeru) sa smie otočiť o 90°
...
| Rez | Obsah | Audit |
|---|---|---|
| **NP-1 algoritmus** | modul `core/cut_plan.rb` (čistý), kontrakt výsledku §3, rozvinutie duplákov, politika otáčania, kerf/orez, RED; headless: determinizmus · duplák ×2/×3 dáva správny počet obdĺžnikov zdroja · dielec so smerom sa neotočí · dielec bez smeru sa otočí podľa pravidla · kerf mení počet platní na hrane · oversize = RED · `fallback` formát · **vlastnosť horná hranica**: pre pripravené prípady plán ≥ známe optimum a ≤ odhad `count_max`; mutácie min. 4 | **ÁNO** (nový modul) |
| **NP-2 sekcia + rozpočet** | Štúdio → Nárezový plán (SVG per platňa, zoznam, príznaky), riadok v rozpočte, prepínač „ceny podľa plánu"; in-SU smoke KLINIKA (počty vs. reálne objednané platne, čas) | NIE (nad kontraktom NP-1), `codex-po-pr` |

## 7 · Nálezy Codex #322 zapracované

kolo 1 P1: heuristika ≠ presné množstvo → **horná hranica**, rozpočet informatívne, objednáva človek · kolo 2 P2: jedna politika otáčania dielcov bez smeru (§1) · kolo 3 P1:
duplák sa rozvinie na `množstvo × multiplier` obdĺžnikov zdroja pred rozkladom (§1) + vlastný test.
