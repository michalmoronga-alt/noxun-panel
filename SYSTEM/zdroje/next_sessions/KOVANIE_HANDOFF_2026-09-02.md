...
**BEŽÍ: PR #277 D-52a (jadro updatera)** — vetva `feat/d52a-updater-core`. Codex kolo 1 = 4×P1 + 1×P2 (boot pri cudzom update locku · smer recovery „strom = vykonávaný loader" · nemazať .old pri neúspešnom rollbacku · latch hneď po commite loadera · duplicitný VERSION celý súbor). Opravy zadané subagentovi. **Ďalší krok pre Opus:** overiť push opráv + reply v threadoch → `gh pr comment 277 --body "@codex review"` (P1 = plné nové kolo povinné) → budík 10 min → ak čisté: `gh pr checks 277` zelené → `gh pr merge 277 --merge --match-head-commit <sha>` → `git checkout main && git pull`. Pravidlo 3 kôl: ak by šlo do 3. kola, rozdeliť PR (skill `codex-po-pr`).
