...
**HOTOVÉ: D-52 KOMPLET (3.9.2026 ráno) — main v0.9.14 (`6214b1a`):** D-52a #277 (jadro) · D-52b1 #278 · D-52b2 #279 (UI + apply). In-SU 1297/0. Uzáver v KRONIKE/STAV/PLAN/DOGFOODING_vyriesene. Plugin v Plugins preinštalovaný z main (0.9.14). **ĎALEJ: KOV-A — AUDIT HOTOVÝ, IMPLEMENTÁCIA EŠTE NIE.** Codex audit návrhu (3.9., checkpoint `KOVANIE_KOVA_AUDIT_2026-09-03_14.md`): 5 BLOCKER + 7 FIX + rez **A1/A2**. Štyri blockery rozhodnuté orchestrátorom technicky (trojstav smeru legacy/unset/left-right; smer dormant s aplikovateľnosťou podľa efektívneho `wings_n`; legacy zásuvka = „neklasifikované"; part_keys `front:F#/flap` a `front:F#/blind`), **jeden čaká na Michala: smer pri 3/4-krídlových dvierkach** (návrh: krajné odvodené, stredné `unset`). **Postup pre Opus:** (1) Michalova odpoveď → (2) prepísať package KOV-A v PLAN.md na A1 + A2 podľa checkpointu #14 (rozhodnutia + FIX 6–12 + minimum testov) → (3) implementácia A1 Opus subagentom vo worktree (bez UI sprístupnenia nových typov), codex-po-pr, in-SU povinné, charakterizácia KLINIKA → (4) A2. **Predpoklad: D-52 (v0.9.14) nasadený aj u Lucie PRED prvým KOV-A modelom.** Potom KOV-B ‖ KOV-H → C → D → E/F/G/I.
