...
má tú istú medzeru — samostatná téma.)* Rovnako sa preskočí skrinka s **kovaním neznámeho typu**: guard prestavby by inak zhodil celú spoločnú operáciu a nezapísala by sa ani
jedna z ostatných skriniek. **Výber dielca prežije:** keď je označené čelo, po prestavbe sa označí jeho náhrada podľa `part_key`, takže karta dielca sa nezavrie, ale ukáže nový
smer — a keď skrinka výberu prestavbou vôbec neprešla (napr. je označený odpojený dielec), výber sa nechá tak, ako je.
...
**Testy:** `tests/pure/test_d131_kresba_ciel.rb` (51 testov), `tests/js/test_d131_ui.js` (38 kontrol), in-SketchUp sekcia `run_d131` (35 kontrol: snapshoty dielcov v modeli,
...
blokáda, cudzí `model_guid` a prázdna zákazka bez kroku Späť). Deväť overených mutácií je v hlavičkách oboch sád.
...
**Vyriešené:** B1 prinieslo štyri osadenia UKW-7, správny rozmer panelu a dĺžku rezu, schému 13, seed 7 a čistenie neaplikovateľných seed zásahov. B2 doplnilo profil/hranu do karty čela a hromadných Úchytiek. Dvierka zvislo vždy oproti pántom; ostatné fyzické typy všetky štyri hrany. Neurčený smer zostáva návrhom v karte, platný zápis sa potvrdí pred exportom, kópiou aj šablónou. Zrušená akcia ani stará odpoveď sa neskôr neuplatní.
**Overenie:** B1 3826 headless / 112 JS / 2295 in-SU PASS, B2 3829 headless / 113 JS a skutočný 470px Inspector s Ruby preflightom. Ručné Redo ostáva používateľským testom. D-114 (ikonový rad) je samostatný uzáver C.
...
- **D-119 · Presah dverí do strán per strana** (Lucia 6.9., prvý test pluginu na jej notebooku) — presah/okraj čela do strán je dnes **jedna hodnota pre obe strany**
  ...
  **Vyriešené: PR #347, v0.10.6 (11.9.2026).** Samostatné ľavé/pravé okraje, kompatibilné čítanie `gap_sides`, config schéma 12, náhľad a dva páry polí. Overené v SketchUpe vrátane Undo, Scale, kópie, šablón a SKP roundtripu.
...
- **D-122 · Kontrola hlási každý UNI dielec zvlášť** (Michal 6.9., zákazka KLINIKA) — Štúdio → Kontrola ukazuje pri UNI farbách **každý dielec ako upozornenie**; pri tvorbe
  ...
  *Pôvodný stav: OTVORENÉ — V1, malá UI dávka v sekcii Kontrola (zoskupenie nálezov podľa príčiny; semafor ostáva ORANGE, neblokuje).*
  **Vyriešené 10.9.2026, PR #343, v0.10.2:** jedna predvolene zbalená skupina „Nenahradené UNI materiály“ s počtom dielcov. Po rozbalení pôvodné hlásenia s vlastníkom,
  okom, ceruzkou a „Nahradiť UNI…“. Skupina sa obnovuje z čerstvých dát; zmena dokumentu alebo odstránenie posledného UNI ruší rozbalenie. Počty semaforu/badge aj exporty
  ostávajú serverové a zahŕňajú jednotlivé nálezy. Implementačné kontroly dopĺňa Michalov večerný smoke.
