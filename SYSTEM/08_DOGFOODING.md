...
- **D-85 · Vyhľadávanie vo VŠETKÝCH selectoch materiálov** (Michal 6.8., test dávky E na reálnej zákazke) — už pri dnešnom malom katalógu je scrollovanie rozbaľovačiek náročné, lebo jedna položka = **dekor × typ × hrúbka** (a s duplákmi ich ešte pribúda). Návrh: (a) jednotný **combobox s písaním** a filtrom **bez diakritiky** — vzor je hotový suggest dropdown z okna Materiály (D-67), takže sa tým zároveň napĺňa odložená **D-16** (autocomplete dekoru); (b) sekcia **„Použité v projekte" navrchu** ponuky (vzor pásu v Materiáloch); (c) **naposledy použité**. Odporúčanie: (a) + (b) ako **jeden zdieľaný komponent** pre všetky miesta výberu — panel (telo / dielec / doska / čelá / chrbát), karta Doska aj projektové predvoľby; nie tri kópie. *Stav: na UI dávku (stredná veľkosť, ide spolu s D-84 a D-89); nahrádza odloženú D-16.*
- **D-89 · Olepenie — ktorá hrana je ktorá strana + „podľa pravidla" nepovie, čo vyšlo** (Michal 6.8., test dávky E na reálnej zákazke) — pri označenom dielci je mätúce, ktorá hrana **L1/L2/W1/W2** je ktorá strana reálneho dielca (človek si to musí domýšľať z orientácie), a údaj **„podľa pravidla"** nehovorí, ČO pravidlo reálne dalo — pri kontrole olepu sa tak nedá overiť nič. Návrh: (a) pri hoveri hrany v karte dielca / ABS editore **zvýrazniť zodpovedajúcu hranu priamo v MODELI** (obojsmerná orientácia), alebo aspoň slovné označenie (predná/zadná/ľavá/pravá podľa roly a orientácie dielca); (b) **„podľa pravidla" rozpísať na resolved hodnotu** — „pravidlo → K2738 23/1, predná pozdĺžna" (server v `resolve_edges` hodnotu už pozná, ide len o zobrazenie). *Stav: na UI dávku spolu s D-84/D-85; (b) je lacná časť a dá sa spraviť samostatne.*

## UX drobnosti (nízka priorita)

- **D-84 · Čelá: „+ riadok / − riadok" pomenovať podľa toho, čo pridávajú** (Michal 6.8., test dávky E na reálnej zákazke) — v karte Čelá má tlačidlo hovoriť rečou stolára: **„+ pridaj čelo"** (zásuvkové) a **„+ pridaj dvere"** (krídlové). Tlačidlo **„− riadok" odpadá úplne** — mazanie ostáva krížikom pri konkrétnom riadku (jednoznačné, ktorý riadok mizne) a zároveň sa uvoľní miesto v rade (pravidlo vertikálneho priestoru). *Stav: malá UI dávka na karte čiel, ide spolu s D-85/D-89.*
- **D-86 · Smer dekoru vo vkladacej karte sa pri refreshi katalógu ticho vráti na predvoľbu** (nález subagenta pri E-03, 6.8.; existuje od V0.4.7c) — vedome zvolené „Bez smeru" (pole `ib_grain`) prepíše živý push z okna Materiály (`NX.setMaterials`) späť na katalógovú predvoľbu materiálu, bez akéhokoľvek náznaku — používateľ vloží dosku s iným smerom, než si nastavil. **Rovnaká trieda chyby ako draft hrúbky v E-03**, takže aj fix je hotový vzor: písať do poľa len vtedy, keď sa reálne zmenil materiál alebo doň používateľ nesiahol (`insertThicknessShouldWrite`). *Stav: zvezie sa s UI dávkou D-84/D-85; nízka škoda, ale tichá — preto sa neodkladá dlho.*
...
- **D-88 · Farba hrany dielca podľa priradenej ABS pásky** (Michal 6.8., test dávky E na reálnej zákazke) — na materiáli Biela 500SM zmenil ABS hranu na K2738, ale hrana v modeli ostala biela; má byť **vizuálne hnedá** (farba dekoru pásky), aby bolo na prvý pohľad vidieť kontrastný olep. Dnes všetky plôšky dielca nesú farbu dosky. Feature: **bočné plôšky farbiť podľa ABS pásky tej hrany**, zdroj farby = skupinová farba dekoru z **D-82**. *Stav: nápad; je to prvý krok „ABS vizuálneho režimu" z vízie V1.0, súvisí s D-87 a M-R.*
...
- **D-16 · Autocomplete dekoru** (Michal 19.7.) — pri výbere materiálu/ABS písať názov z katalógu, návrhy sa dopĺňajú za každým písmenom, → a Enter potvrdí. *Stav: **NAHRADENÉ D-85** (6.8.) — z „nice to have" sa testom na reálnej zákazke stal spomaľovač; rieši sa ako jeden zdieľaný combobox pre všetky selecty materiálov.*
