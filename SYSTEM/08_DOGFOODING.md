...
- **D-43 · Zdvojené dosky (duplák 36 = 2× 18)** (Michal 27.7.) — **(a) overené v kóde (Fable 27.7.):** logika „36 = 2×18" dnes NEEXISTUJE nikde — kusovník aj odhad platní (`sheet_estimate.rb`) zoskupujú podľa `material_id`, takže dielec 36 spotrebuje 1× svoju plochu z platne varianty „36"; VEPO merge 18+36 je len spoločný SÚBOR (dielec ide 1× s hrúbkou 36 — zhodné so starou linkou). Dôsledok: ak sa duplák lepí z 2×18, interný odhad platní podhodnocuje spotrebu 18-tky. **(b) návrh modelu:** variant 36 označený ako „zdvojený z 18" (odkaz na zdrojový variant + multiplikátor ×2, BEZ vlastného DK kódu — kupuje sa 18-ka) → odhad platní pripočíta 2× plochu do skupiny 18; pri pridaní 18 varianty auto-ponúknuť vytvorenie zdvojenej 36 (Halifax: 18,6 → 37,2). *Stav: na návrhovú dávku (sedenie ③ / V0.6).*
- **D-44 · Rýchly výber výrobcu a typu pri zadávaní** (Michal 27.7.) — pri batchi/edite materiálu vyberať výrobcu rozkliknutím z už zadaných + možnosť dopísať nového; to isté pre TYP dosiek (DTDL, PD 600, PD 920…) — úsporne (pravidlo vertikálneho priestoru). Poznámka: typy „PD 600/PD 920" prirodzene riešia aj identitu PD variantov rovnakej hrúbky (otázka 3 v 09_POJMY — šírka ako súčasť typu). *Stav: na UX dávku (spolu s D-15 vzorom).*
