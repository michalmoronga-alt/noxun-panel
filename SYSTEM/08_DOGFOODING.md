...
>
> **Stav k 3.8.:** smoke test V1 uzavretý (fiktívna zákazka, 12 skriniek + 4 dosky — kovanie checklist **6/6** vrátane „Z Demosu" a CSV exportu, semafor ČISTÝ po nahradení UNI reálnymi dekormi). Nálezy testu sú roztriedené nižšie ako **D-75 až D-83** (dávky H1–H4), väčšie celky sú spísané vo vízii uzatvorenia V1: [10_V1_VIZIA.md](10_V1_VIZIA.md).
...
- **D-79 · Nohy: default je klzák aj pri sokli 150** (Michal 3.8., smoke C-02 — skrinka so soklom 150 dostala „Klzák 17 mm") — klzák patrí LEN ku skrinkám bez sokla (výška 17–21 mm), inak je prakticky zle. Dve časti: (a) seed default nôh prepnúť na **AXILO**; (b) systémovo = noha ako **RAD podľa výšky sokla** s pásmami (17–21 klzák · AXILO 100 · 120 · 150; AXILO majú rektifikačný rozsah, preto pásma a nie presný kľúč), mimo pásiem ORANGE. Výška sokla je v expanzii pravidla už dnes k dispozícii (`params.height`). *Dávka: H1 (kovanie).*
- **D-80 · Vnútorný priestor nerešpektuje odsadené výstuhy** (Michal 3.8., varná skrinka — „odsadené nadnožie zmenšuje vnútorný priestor, plugin by s tým mal rátať") — `Construction.interior_dims` ignoruje `rails_top_offset` aj výšku výstuh postavených na výšku (upright), takže zóny, police a šuflíky sa počítajú do pásma, ktoré zaberá varná doska. Správne: **z_hi = SPODNÁ hrana výstuh** (flat: výška − offset − hrúbka; upright: výška − offset − hĺbka výstuhy — pri upright je diera aj bez offsetu). Potvrdené živo: offset 30 → zóna hlási vnútro 674 namiesto 644. Pri oprave pozor na väzby: to isté z_hi používa chrbát inset/groove (D-37) + SU testy geometrie. *Dávka: H3 (geometria).*
- **D-75 · Nový set kovania sa v selecte na skrinke objaví až po reštarte UI** (Michal 3.8., smoke A-01 — set „chladničkový" sa dal zvoliť „až po reštarte UI pluginu") — po CRUD v Katalógu kovania chýba **živý push setov do panela** (vzor `NX.setMaterials` — materiály živý sync majú). Overiť rozsah aj pre tab Predvoľby projektu. *Dávka: H1 (kovanie).*
- **D-81 · Výsuv per ČELO — výška bočnice** (Michal 3.8., varná skrinka: horné čelo potrebuje Atira H70, ostatné H174) — override setu **per SKRINKU už existuje** (D1b), chýba per ČELO. Cieľ: **automatika pásiem** — výška čela z veľkej časti určuje výšku bočnice (H70/H144/H176; vzor fit_series „systém vyberá") + **owner-level override** na výnimky ({(generic_type, owner_part_key) → set_id}, select v kovacom riadku čela). Pásma výšok dodá Michal pri návrhu dávky. *Dávka: H1 (kovanie).*
- **D-76 · Šablóna má niesť kovanie + oprava merge_template** (Michal 3.8., smoke A-02/C-01 — cieľ „chladničková skrinka s kovaním" ako položka knižnice) — šablóny dnes `hardware_sets` neukladajú a `merge_template` kopíruje z cieľa len `part_overrides` a `hardware_overrides` → **aplikácia šablóny ticho ZMAŽE override setu na cieľovej skrinke**. Fix: `hardware_sets` doplniť do `template_config_from` aj `merge_template` (mapa typ→set_id nie je viazaná na dielce zdroja, prenáša sa bezpečne); ak set v novom projekte neexistuje = ORANGE (missing ≠ invalid, nikdy tichý fallback). `hardware_overrides` do šablón zvážiť samostatne. *Dávka: H2 (šablóny).*

## UX drobnosti (nízka priorita)

- **D-77 · Okno detailu dielca je po otvorení odseknuté** (Michal 3.8.) — pravá spodná časť nastavení dielca nie je vidieť, kým sa okno ručne nezväčší. *Čaká upresnenie, o KTORÉ okno ide (karta dielca v paneli vs detail v Materiáloch) — Michal dodá screenshot. Dávka: H4 (UI).*
- **D-78 · Po prepnutí vkladacej karty na „Doska" zmiznú taby hlavičky** (Michal 3.8.) — taby Korpus·Zóny·Čelá zmiznú a nevrátia sa ani klikom do prázdna, len prepnutím späť na „Korpus". Príčina: vkladací prepínač si voľbu pamätá v `data-insert-kind` na body a CSS skrýva `.cabtab` aj v `mode-insert` s kind=board. *Fix: zjednotiť viditeľnosť tabov v mode-insert bez ohľadu na pamätaný vkladací kind. Dávka: H4 (UI).*
- **D-82 · Farba je vlastnosť DEKORU, nie jedného variantu** (Michal 3.8. večer) — farba zobrazenia sa dnes mení per variant, hoci celá dekorová skupina má vždy rovnakú farebnosť; výsledok je „projekt je hnedé more, všetko default hnedé". Riešenie: skupinová atomická operácia `set_decor_color` (mechanika `rename_decor` / `set_decor_manufacturer` už existuje), buildery farbia SU materiál zo skupiny. *Výhľad M-R: Demos fotka dekoru ako TEXTÚRA (lokálna image cache už existuje — quick-win), farba ostane fallback. Dávka: H4 (UI/materiály).*
- **D-83 · „Nahradiť UNI…" sa pri plnej zákazke ťažko hľadá** (Michal 3.8. večer) — je náročné zistiť, KTORÝ UNI materiál kde je použitý, a dostať sa k nahradeniu. Sprístupniť akciu priamo z **KONTROLA tabu**: klik na ORANGE „materiál neurčený" ponúkne „Nahradiť UNI…" s predvyplneným zdrojom. *Súvis: pás „Použité v projekte" v Materiáloch — prepojiť. Dávka: H4 (UI).*
