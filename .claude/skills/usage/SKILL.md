...
1. **Na začiatku okna** (spolu s STAV/PLAN): `usage.ps1` bez parametrov — do prvej správy Michalovi jednou vetou, ak niečo obmedzuje plán (napr. Codex weekly > 85 %). Je to prvá časť kontroly štartu okna; ďalšie dve (lokálne nástroje a register `agent-register`) popisuje CLAUDE.md, sekcia Kvóty a štart okna — celú kontrolu naraz spustí `powershell -NoProfile -File scripts\start_okna.ps1`.
