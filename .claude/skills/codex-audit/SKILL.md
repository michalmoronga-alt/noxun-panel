...
**Rozdelenie modelov (Michal 5.9. a 13.9.2026 — model podľa zložitosti):** **Astra (`gpt-6-astra`)** = predvolený model v `~/.codex/config.toml` (od 13.9.), používa sa na audity návrhov meniacich dátový kontrakt/schému/migráciu/observer-undo lifecycle, nové moduly, nové SketchUp API plochy (audit-povinná trieda) a ako tretí hlas pri sporných nálezoch; **Sol** (`task --background --model gpt-5.6-sol <prompt>`) = bežné audity, delta verifikácie po opravách, opakované kolá, UI dávky. Astra ukrajuje z Codex limitu viac — nie na rutinu.
**Verzia CLI:** Astra vyžaduje Codex CLI ≥ 0.154 — `codex --version` musí hlásiť npm balík z `%APPDATA%\npm` (0.154+), nie desktop-app binárku `…\Programs\OpenAI\Codex\bin\codex.exe` (0.144, PATH bol 13.9. preusporiadaný). Companion spúšťa `codex app-server` z PATH a drží ho ako broker; po upgrade CLI treba starý broker (`node …app-server-broker.mjs` + `codex app-server`) zastaviť, inak beží stará verzia ďalej. V okne otvorenom pred zmenou PATH: `$env:Path = "$env:APPDATA\npm;" + $env:Path` pred volaním companiona. GH PR review model je na strane Codex cloudu. Proces sa osvedčil: pred+po Codex chytil dokopy 5+ blockerov a 5 reálnych bugov.
**Kvóta (od 13.9.2026, skill `usage`):** pred odoslaním úlohy spusti `& ".claude\skills\usage\usage.ps1" -Label "audit <dávka>" -Phase before -Gate codex`; **exit 3 = Codex weekly pod 10 % → audit neposielaj**, ohlás to a pokračuj s prísnejšou vlastnou kontrolou návrhu (alebo audit odlož po resete). Po výsledku `-Phase after` (log spotreby).
