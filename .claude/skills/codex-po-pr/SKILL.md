...
0. **Pred `gh pr create`:**
   - **Predrecenzia** (skill `predrecenzia`) prebehla pri dávke **audit-povinnej**, **výrobnej/cenovej** (definícia v CLAUDE.md) a pri **bežnej dávke nad
     300 zmenených riadkov kódu pluginu** (bez testov a dokumentácie) **alebo s novým ovládacím prvkom v UI** — jej P1/P2 sú opravené a PR popis má sekciu
     „Predrecenzia". Pri docs-only sa nerobí.
   - **Kvótová brána (rozhodnutie N18, 26.9.2026):** `& ".claude\skills\usage\usage.ps1" -Label "<dávka>" -Phase before -Gate codex` — **exit 3 (Codex weekly
     zostatok < 10 %) = PR sa otvorí ako draft** (`gh pr create --draft`; Codex draft nerecenzuje) a platí **náhradná brána** (krok 3); exit 2/4 neblokujú
     (rozhodni ručne). Dôvod: kolo, ktoré beží samo po otvorení PR, sa nedá zastaviť.
...
3. **Každý nález:** posúď závažnosť (P1/P2/P3). **Opravu robí pôvodný implementátor** (pokračovanie toho istého subagenta — pozná kód aj dôvody; jeho
   ...
   **Kvótová brána pred každým ďalším kolom (skill `usage`):** pred `gh pr ready` aj pred každým `@codex review` spusti `& ".claude\skills\usage\usage.ps1" -Gate codex` (číta len Codex) — **exit 3 (Codex weekly zostatok < 10 %) = kolo NEVYŽIADAŠ**; exit 2/4 neblokujú (rozhodni ručne). Platí **náhradná brána**: slepý recenzent s reprodukciami + interná delta-verifikácia (vzor 31.8., 9.9. a PR #363 13.9.2026) a do PR zapíšeš komentár, že GH kolo nahradila z dôvodu kvóty. Výnimka: audit-povinná alebo výrobná/cenová dávka a P0/P1 → spýtaj sa Michala, môže kolo povoliť aj pod prahom (v autonómnom behu: kým neodpovie, dávka čaká a pokračuje sa ďalšou nezávislou — tabuľka v CLAUDE.md). Kolo, ktoré beží samo po otvorení PR alebo po `gh pr ready`, sa nedá zastaviť — ak zlyhá na limite (bot „Failed"/ticho), ber to ako nevyžiadané a použi náhradnú bránu.
