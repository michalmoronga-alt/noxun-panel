...
8. **Zápis `dynamic_attributes` v operácii, ktorá entity VYTVÁRA = mŕtve selection eventy (D-40, 21.7.2026).** Ak nová definícia/inštancia vznikne a dostane DC atribút (napr. `scaletool`) v TEJ ISTEJ operácii, DC extension observer pri commite (dávkovaný `onElementAdded` entity s `dynamic_attributes`) spôsobí, že SketchUp PRESTANE doručovať selection observer eventy celému modelu — až do zmeny edit kontextu (vstup/výstup z komponentu). Preto DC atribúty na čerstvo vytvorené entity zapisovať až v SAMOSTATNEJ TRANSPARENTNEJ operácii hneď za vložením (`start_operation(n, true, false, true)`; vzor `apply_scale_lock_op` v builderoch). **Transparentnú operáciu NIKDY neabortovať** — abort ruší aj prilepenú predchádzajúcu operáciu (SketchUp API); pri chybe commitnúť aj čiastočný zápis. Pôvodné meranie bezpečných kópií/rebuildov nepokrývalo vnorený `make_unique`. **MR-3B (12.9.2026):** kopírované DC atribúty na nových vnorených inštanciách alebo definíciách vypínajú selection eventy aj bez explicitného DC zápisu; výpadok s `disable_ui=true` trvá aj po UI ticku. Apply preto používa jednu normálnu operáciu s `disable_ui=false` pod pôvodným `ScaleWatch.guard`, s úplným abortom a jedným Undo krokom. Natívny probe porovnal obe hodnoty na DC definícii, inštancii aj oboch. Nepridávať liečivú prázdnu operáciu ani prepínanie edit kontextu — zakryli by chybu a narušili meranie.
