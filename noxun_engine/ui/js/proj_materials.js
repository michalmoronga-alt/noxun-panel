  ...
  // 2A-1: SCHEMA katalogu z posledneho payloadu. Vracia sa v KAZDEJ mutacii —
  // po migracii na SCHEMA 2 server odmietne zapis z okna, ktore este bezi na
  // starom kode (CEF cache). 1 = legacy, pokial server nepovie inak.
  var MD_SCHEMA = 1;
  ...
  function mdCellFlush(inp){
    ...
    var payload = {
      ...
      catalog_schema: MD_SCHEMA, // 2A-1: row_rev strazi riadok, schema strazi format katalogu
  ...
  function mdRenameSave(oldDecor){
    ...
    if (window.sketchup && sketchup.rename_decor)
      sketchup.rename_decor(JSON.stringify({ old_decor: oldDecor, new_decor: input.value, catalog_rev: MD_REV, catalog_schema: MD_SCHEMA }));
    mdRenaming = null;
  }
  ...
  function mdManufacturerSave(decor){
    ...
    if (window.sketchup && sketchup.set_decor_manufacturer)
      sketchup.set_decor_manufacturer(JSON.stringify({ decor: decor, manufacturer: input.value, catalog_rev: MD_REV, catalog_schema: MD_SCHEMA }));
  ...
  function mdManufacturerClear(decor){
    if (window.sketchup && sketchup.set_decor_manufacturer)
      sketchup.set_decor_manufacturer(JSON.stringify({ decor: decor, manufacturer: '',
        clear_manufacturer: true, catalog_rev: MD_REV, catalog_schema: MD_SCHEMA }));
    mdManufacturing = null;
  }
  ...
  function mdSaveDecorBatch(){
    ...
    var payload = {
      ...
      catalog_rev: MD_REV, catalog_schema: MD_SCHEMA,
  ...
  function mdDeleteSheet(id){
    if (window.sketchup && sketchup.delete_sheet) sketchup.delete_sheet(JSON.stringify({ material_id: id, catalog_rev: MD_REV, catalog_schema: MD_SCHEMA }));
  }
  function mdDeleteEdge(id){
    if (window.sketchup && sketchup.delete_edge) sketchup.delete_edge(JSON.stringify({ abs_id: id, catalog_rev: MD_REV, catalog_schema: MD_SCHEMA }));
  }
  ...
  function mdApplyCatalog(data){
    ...
    MD_SCHEMA = data.catalog_schema || 1; // 2A-1: schema katalogu (guard mutacii)
