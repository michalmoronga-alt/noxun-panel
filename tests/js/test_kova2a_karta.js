...
C.FRONT_CARD_TYPES.forEach(t => {
  const m = C.frontCardModel({ type: t }, []);
...
const unk = C.frontCardModel({ type: 'sliding_2027' }, []);
...
eq(rowKeys(C.frontCardModel({ type: 'door' }, [slot('single', null)])),
   ['direction:single', 'opening_mode'], '1 kridlo: smer + otvaranie');
eq(rowKeys(C.frontCardModel({ type: 'door' }, [])),
   ['opening_mode'], '2 kridla (server nekladie otazku): ZIADNY riadok smeru');
ok(infoTexts(C.frontCardModel({ type: 'door' }, [])).some(t => t.indexOf('Dvojkrídlo') === 0),
   'namiesto neho sa povie, co plati (lave = panty vlavo, prave vpravo)');
eq(rowKeys(C.frontCardModel({ type: 'door' }, [slot('p2', null)])),
   ['wing_direction:p2', 'opening_mode'], '3 kridla: len STREDNE kridlo');
eq(rowKeys(C.frontCardModel({ type: 'door' }, [slot('p2', null), slot('p3', null)])),
...
eq(C.frontCardModel({ type: 'door' }, [slot('p2', null)]).rows[0].label, 'Krídlo 2/3',
   '3 kridla: p2 je „Krídlo 2/3"');
eq(C.frontCardModel({ type: 'door' }, [slot('p2', null), slot('p3', null)]).rows.map(r => r.label).slice(0, 2),
...
eq(rowKeys(C.frontCardModel({ type: 'door' }, null)), ['opening_mode'],
   'bez slotov sa smer NEPYTA (a ani sa netvrdi, ze je dvojkridlo)');
eq(infoTexts(C.frontCardModel({ type: 'door' }, null)), [],
   'a nekresli sa ani veta o dvojkridle');

// --- ostatne typy ---------------------------------------------------------
eq(rowKeys(C.frontCardModel({ type: 'drawer_front' }, [])),
   ['opening_mode', 'drawer_construction', 'drawer_variant'], 'zasuvka: otvaranie + klasifikacia');
eq(rowKeys(C.frontCardModel({ type: 'lift' }, [])), ['opening_mode'], 'vyklop: len otvaranie');
eq(rowKeys(C.frontCardModel({ type: 'fall' }, [])), ['opening_mode'], 'sklop: len otvaranie');
eq(rowKeys(C.frontCardModel({ type: 'blind' }, [])), [], 'blenda nema smer ani otvaranie');
eq(rowKeys(C.frontCardModel({ type: 'none' }, [])), [], '„Bez čela" nema co nastavovat');
ok(infoTexts(C.frontCardModel({ type: 'blind' }, [])).some(t => t.indexOf('pevný výrobný dielec') > 0),
...
eq(rowKeys(C.frontCardModel({ type: 'drawer_front', direction: 'left' }, [slot('single', 'left')])),
...
eq(activeOf(C.frontCardModel({ type: 'door' }, [slot('single', null)]), 'direction:single'), null,
   'LEGACY celo (kluc smeru chyba): ziadna aktivna volba');
eq(activeOf(C.frontCardModel({ type: 'door', direction: UNSET }, [slot('single', UNSET)]), 'direction:single'),
   UNSET, 'vedome neurcene: aktivna je volba „Neurčené"');
eq(activeOf(C.frontCardModel({ type: 'door', direction: 'left' }, [slot('single', 'left')]), 'direction:single'),
   'left', 'vyriesene: aktivna je strana');
eq(activeOf(C.frontCardModel({ type: 'door', wing_directions: { p2: 'right' } }, [slot('p2', 'right')]),
            'wing_direction:p2'), 'right', 'stredne kridlo cita svoju hodnotu z wing_directions');
eq(activeOf(C.frontCardModel({ type: 'door', direction: 'left' }, [slot('p2', null)]), 'wing_direction:p2'), null,
   'scalarny smer sa pri 3/4 kridlach NECITA (dormant)');
eq(activeOf(C.frontCardModel({ type: 'door' }, []), 'opening_mode'), null,
   'chybajuce otvaranie = ziadna aktivna volba (NIE „klasicke")');
ok(C.frontCardModel({ type: 'door' }, []).rows.find(r => r.key === 'opening_mode').hint.indexOf('predvolene') > 0,
   'namiesto tichej volby sa povie, co plati, kym to nikto neurci');
const drwLegacy = C.frontCardModel({ type: 'drawer_front' }, []);
...
eq(infoTexts(C.frontCardModel({ type: 'drawer_front', drawer: { construction: 'metal' } }, [])), [],
...
C.frontCardModel(legacyItem, [slot('single', null)]);
...
global.frontSlots = { F1: [slot('single', null)], F2: [], F3: [slot('single', UNSET)] };
...
global.frontSlots = { F1: [slot('single', null)] };
openCard('F1');
...
global.frontSlots = { F1: [slot('single', 'left')] };
